...
- **Overview** condenses the scan into a one-page report you can read top to bottom or print: an overall compliance headline, score by level (L1 / L2 / BitLocker), a trend across recent scans, the weakest categories, and what changed since last time.
- **Console** is where you work through the findings: search, filter, and sort every recommendation, save views you return to, and open a detail drawer with the expected and found values for each check.
